...
email_field = browser.find_element(by=By.NAME, value="LoginForm[login]")
email_field.send_keys("yyurchikoff@gmail.com")

# делаем скриншот
browser.save_screenshot("screenshotas.png")
time.sleep(5)
# Находим поле ввода пароля и вводим пароль
password_field = browser.find_element(by=By.NAME, value="LoginForm[password]")
password_field.send_keys("ipwj4t")


# делаем скриншот
browser.save_screenshot("screenshotlog.png")
...
async def get_syte(id_user, link):
    try:
        try:
            ...
            # делаем скриншот
            browser.save_screenshot("sssscreenshot.png")
            browser.get(link)
            # делаем скриншот
            browser.save_screenshot("screenshotlink.png")
            await asyncio.sleep(5)
